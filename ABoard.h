...
public:
	...
	bool checkMate() { //comparing kings moves to the possible
		kingCheck(map, turn);
		if (!noMoves()) {
			game_over = true;
		}
	...
	bool kingCheck(vector<Piece*>& map, bool turn) {
		...
		for (int pieceIndex = 0; pieceIndex < absoluteSize; pieceIndex++) {
			if (map.at(pieceIndex)->team == flip && map.at(pieceIndex)->pieceInt == 4) {
				...
				kingVec.at(pieceIndex) = true; //add kings loc to vector

				if (checkVector(map, turn).at(pieceIndex) && kingVec.at(pieceIndex)) { 
					kingCheck = true;
