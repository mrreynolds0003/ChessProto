...
public:
	...
	bool isCheckmate(vector<Piece*>& map, bool turn) {
		// First, check if the king is in check
		if (!kingInCheck(map, turn)) {
			return false; // King is not in check, so it's not checkmate
		}

		// Iterate over all pieces of the current player
		for (int i = 0; i < absoluteSize; i++) {
			if (map.at(i)->team == (turn ? WHITE_TEAM : BLACK_TEAM)) {
				// Check all possible moves for this piece
				for (int j = 0; j < absoluteSize; j++) {
					vector<bool> possibleMoves;
					if (dynamic_cast<Pawn*>(map.at(i))) {
						possibleMoves = dynamic_cast<Pawn*>(map.at(i))->captureMoves(map, i);
					}
					else {
	...
	bool kingInCheck(vector<Piece*>& map, bool turn) {
		bool inCheck = false;
		vector <bool> checkReturn = allMovesVector(map, turn);
					...
					if (turn == true && map.at(currPieceLoc)->team == WHITE_TEAM) { ///white team
						if (map.at(currPieceLoc)->move(map, currPieceLoc).at(nextPieceLocation)) {
							if (futureCheck(currPieceLoc, nextPieceLocation)) {
								...
								turn = false;
								// Inside your main game loop after each move
								if (isCheckmate(map, turn)) {
									game_over = true;
								}
							}
						}
...
public:
	...
	bool outOfMoves() {
		...
		//def of no piece can bail him out:
			//no piece can make it so the king's position is not aligned with a true boolean in checkvector
				...
				bool mate = true;
				for (int i = 0; i < absoluteSize; i++) {
					if (map.at(i)->team == flip) {
						for (int k = 0; k < absoluteSize; k++) {
							vector <bool> piecesMoves = allMovesVector(map, turn);
							if (futureCheck(piecesMoves.at(i), piecesMoves.at(k))) {
								mate = false;
	...
	bool checkMate(int currPieceLoc, int nextPieceLoc) {
		...
		if (kingInCheck(map, turn)) {
			if (!outOfMoves() && !futureCheck(currPieceLoc, nextPieceLoc)) {
				inCheckMate = true;
			}
		}
		return inCheckMate;
